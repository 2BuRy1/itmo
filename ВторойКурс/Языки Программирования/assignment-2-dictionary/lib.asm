...
string_length:
    xor rax, rax              ; Обнуляем rax (для подсчёта длины строки)
...
print_string:
    ...
    call string_length         ; Получаем длину строки (возвращается в rax)
    pop rsi                    ; Восстанавливаем указатель на начало строки в rsi
    mov rdx, rax               ; Копируем длину строки в rdx
    mov rax, 1                 ; Системный вызов write (код 1)
    mov rdi, 1                 ; Стандартный вывод (stdout)
