                ...
                for (String line = ScriptExecutorManager.readfile(); line != null; line = ScriptExecutorManager.readfile()) {
                    ...
                    if (commandManager.getCommands().get(cmd[0]) != null) {
                        if(cmd[0].equals("remove_by_id")){
                            stringBuilder.append(commandManager.execute(new Request(commandManager.getCommands().get(cmd[0]),Integer.parseInt(cmd[1].trim()))).getResult()).append("\n\n");
                        }
                        if(cmd[0].equals("add") || cmd[0].equals("add_if_min")){
                            SpaceMarine spaceMarine = new SpaceMarineBuilder().create();
                            stringBuilder.append(commandManager.execute(new Request(commandManager.getCommands().get(cmd[0]), spaceMarine)).getResult()).append("\n\n");
                        }
                        if(cmd[0].equals("insert_at")  || cmd[0].equals("update")){
                            SpaceMarine spaceMarine = new SpaceMarineBuilder().create();
                            stringBuilder.append(commandManager.execute(new Request(commandManager.getCommands().get(cmd[0]), spaceMarine, Integer.parseInt(cmd[1].trim()))).getResult()).append("\n\n");

                        }
                        else {
                            stringBuilder.append(commandManager.execute(new Request(commandManager.getCommands().get(cmd[0]),cmd[1])).getResult()).append("\n\n");
                        }
                    }
                    if(cmd[0].equals("execute_script")){
